...
contract DrugLedger {
    ...
    function register(
        ...
        require(
            bytes(manufacturers[_addr].name).length == 0,
            "Manufacturer already registered"
        );
        manufacturers[_addr] = Manufacturer({name: _name, license: _license});
    ...
    function retrieve(uint256 _id) external view returns (Drug memory) {
        return drugs[_id];
    }
    ...
    function closeIssue(
        ...
        require(
            drugs[_drugId].issues[_issueId].owner == msg.sender,
            "Must be owner"
        );
    ...
    function verify(uint256 _drugId) external view returns (bool) {
